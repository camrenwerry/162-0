...
assert.match(runbook, /wrangler pages deployment tail --project-name pennant-pursuit-preview --environment preview --format json --search preview\.operation/)
assert.match(runbook, /three consecutive `server-error` events/)
assert.match(runbook, /five `rate-limited` events/)
assert.match(runbook, /Retained, privacy-reviewed aggregation.*Milestone 3C-2/s)
