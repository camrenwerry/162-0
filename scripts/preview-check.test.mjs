...
test('Workerd timing-safe-equality is part of the aggregate test path', () => {
  ...
  assert.equal(
    ...
    'npm run test:release-observation-contracts && npm run test:release-preview-observation && npm run test:preview-resource-observation && npm run test:preview-capability-projection && npm run test:release-observation-declarations',
  )
...
test('all recursively reachable release scripts use supported local-only commands', () => {
  ...
  assert.equal(reachable.some(({ name }) => name === 'test:release-preview-observation'), true)
  assert.equal(reachable.some(({ name }) => name === 'test:preview-resource-observation'), true)
  assert.equal(reachable.some(({ name }) => name === 'test:preview-capability-projection'), true)
