...
const lazyRouteSource = source('src/utils/lazyRoute.ts')
const recoverySource = source('src/components/AppRecovery.tsx')
assert.match(appSource, /importLazyRoute\(\(\) => import\('\.\/components\/draft\/ClassicMode'\)\)/)
assert.match(appSource, /importLazyRoute\(\(\) => import\('\.\/components\/leaderboard\/LeaderboardScreen'\)\)/)
assert.match(lazyRouteSource, /pennant-pursuit:chunk-recovery/)
assert.match(lazyRouteSource, /sessionStorage/)
assert.match(lazyRouteSource, /registration\.update\(\)/)
assert.match(lazyRouteSource, /controllerchange/)
assert.match(lazyRouteSource, /CHUNK_RECOVERY_DEADLINE_MS = 8_000/)
assert.match(recoverySource, /obsolete app file after one automatic recovery attempt/)
