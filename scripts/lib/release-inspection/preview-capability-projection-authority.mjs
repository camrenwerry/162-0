...
const validatedInputs = createReleaseInspectionWeakMap()
...
const OPERATIONS = new Set(PREVIEW_OPERATION_INVENTORY)
const OPERATION_ORDER = new Map(PREVIEW_OPERATION_INVENTORY.map((operation, index) => [operation, index]))
...
function assertCanonicalBudget(input) {
  ...
  const add = (bytes) => {
    total += bytes
    if (total > PREVIEW_OBSERVATION_LIMITS.maximumSerializedObservationBytes) {
      fail('serialized snapshot exceeds the approved byte limit.')
    }
  }
...
function authorize(snapshot) {
  setReleaseInspectionWeakMapValue(validatedInputs, snapshot, snapshot)
  return snapshot
}

// This is the only authority-creation boundary. It accepts untrusted data only
// through the complete single-read validator and never exposes registration.
export function validateAndAuthorizePreviewSingleReadSnapshot(input) {
  ...
  return authorize(immutablePlain({ ...snapshot, resourceOutcomes: outcomes }))
}

export function createAndAuthorizePreviewSingleReadSnapshot(input = {}) {
  assertReleaseInspectionIntrinsicIntegrity()
  assertCanonicalBudget(input)
  ...
  return validateAndAuthorizePreviewSingleReadSnapshot({
...
export function resolveValidatedPreviewCapabilityProjectionInput(input) {
  assertReleaseInspectionIntrinsicIntegrity()
  const snapshot = getReleaseInspectionWeakMapValue(validatedInputs, input)
  if (snapshot === undefined) {
    throw new TypeError('Preview capability projection refused: input is not an opaque validated single-read snapshot.')
  }
  return snapshot
}
